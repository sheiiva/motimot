...
- [ ] **1.1** Display the grid: `GameConstants.MaxAttempts` rows × `GameConstants.WordLength` tiles (e.g. 6×5).
- [ ] **1.2** Bind grid to `GameState`: show submitted attempts (rows with letters and feedback) and the current row in progress (letters only).
- [ ] **1.3** Show letters in each tile (from `Row.Tiles` or `CurrentRowLetters`).
